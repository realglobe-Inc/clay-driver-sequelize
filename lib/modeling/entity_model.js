  ...
  const Entity = db.define(modelName, Object.assign(
    ...
    instanceMethods: {
      valueFor (col, {extras} = {}) {
        ...
        const extraVr = extra && extra.vr
        if (extraVr) {
          const vrMisMatch = extraVr !== s.vr
          if (vrMisMatch) {
            throw new Error(`Data collapsed: ${valueName}`)
          }
        }
        const value = extra && extra.value ? (s[valueName] + extra.value) : s[valueName]
        if (!type) {
          return value
        }
      ...
      async updateExtra (extraValues, vr) {
        const s = this
        const extraNames = Object.keys(extraValues)
        if (extraNames.length === 0) {
          return
    ...
    classMethods: {
      ...
      async forList (attributes, {offset, limit, filter, sort} = {}) {
        ...
        return s.findAndCountAll({
          ...
          where: parseFilter(filter, {attributes, valueBaseLength}),
      ...
      async forOne (attributes, cid) {
        ...
        const found = await Entity.findOne({
          attributes: ['vr', ...attributeValuesWith(attributes)],
          include: [
            {model: Entity.Extra, as: extraRefName}
          ],
          where: {cid}
