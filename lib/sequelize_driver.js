...
class SequelizeDriver extends Driver {
        ...
        retry(() => db.transaction({}, task), {
          ...
          report (msg, options) {
            if (options.$current > 1) {
              unlessProduction(() => console.warn(msg))
            }
          },
  ...
  getEntityModels (resource) {
    ...
    return co(function * () {
      ...
      const Entity = entityModel({ db, EntityAttribute, resourceName, prefix })
      yield Resource.sync()
      yield Entity.sync()
  ...
  one (resourceName, id) {
    ...
    return co(function * () {
      ...
      let cid = String(id)
      let entity = yield Entity.forOne(cid)
      if (!entity) {
  ...
  list (resourceName, condition = {}) {
    ...
    return co(function * () {
      ...
      let { rows, count } = yield Entity.forList({ offset, limit, filter, sort })
  ...
  update (resourceName, id, attributes) {
    ...
    return co(function * () {
      ...
      const { Entity, EntityAttribute } = yield s.getEntityModels(resource)
      let cid = String(id)
      const entity = yield Entity.forOne(cid)
      Entity.forOneCache.del(Entity.cacheKeyFor(cid))
