    ...
    instanceMethods: {
      asClay () {
        ...
        let values = s.EntityAttributes.reduce((attributes, entityAttribute) => {
          ...
          let isArray = ARRAY_INDEX_PATTERN.test(name)
          let value = entityAttribute.toValue()
          if (isArray) {
            let [ , arrayName, , index ] = name.match(ARRAY_INDEX_PATTERN)
            attributes[ arrayName ] = attributes[ arrayName ] || []
            attributes[ arrayName ][ Number(index) ] = value
            return attributes
          } else {
