...
class SequelizeDriver extends Driver {
  ...
  async getEntityModels (resource) {
    ...
    return s._modelWorking[prefix] = (async () => {
      const {db} = s
      const Lock = await s.getLockModel()
  ...
  async getLockModel () {
    const s = this
    if (s._models.Lock) {
      return await s._models.Lock
    }
    s._models.Lock = (async () => {
      const {db} = s
  ...
  async close () {
    ...
    await Promise.all(Object.values(s._modelWorking))
    await s._models.Resource
    await s._models.Usage
    await s._models.Lock
