...
class SequelizeDriver extends Driver {
  ...
  async getUsageModel () {
    ...
    this._models.Usage = (async () => {
      const {db} = this
      const Usage = usageModel({db, varDir: this._varDir})
      await Usage.sync()
  ...
  async getLockModel () {
    ...
    this._models.Lock = (async () => {
      const {db} = this
      const Lock = lockModel({db, varDir: this._varDir})
      await Lock.sync()
      if (cluster.isMaster) {
        await asleep(10)
  ...
  async one (resourceName, id) {
    ...
    throw new Error(`[ClayDriverSequelize]One failed ${resourceName}#${id}`)
  }
  ...
  async update (resourceName, id, attributes) {
    ...
    const entity = await this.do(resourceName, async ({Lock, resource}) => {
      ...
      const entity = await Entity.forOne(entityAttributes, cid)
      if (!entity) {
        throw new Error(`[ClayDriverSequelize][${resourceName}] Entity not found for id: ${id}`)
      }
