...
const ALL_CACHE_DURATION = 10 * 1000
...
function attributeModel ({db, prefix = ''}) {
  ...
  const Attribute = db.define(modelName, {
    ...
    classMethods: {
      async allOf () {
        const hasCache = Attribute.allCache && (new Date() - Attribute.allCacheAt < ALL_CACHE_DURATION)
        if (hasCache) {
          return Attribute.allCache
        }
        const found = await Attribute.findAll()
        Attribute.allCache = found
        Attribute.allCacheAt = new Date()
