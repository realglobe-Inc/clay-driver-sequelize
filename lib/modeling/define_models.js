    ...
    classMethods: {
      forList (resource, { offset, limit, filter, sort } = {}) {
        ...
        return co(function * () {
          ...
          let selectSQL = [
            'SELECT `Entity`.*, ',
            [ 'id', ...Object.keys(EntityAttribute.attributes) ].map((name) => '`EntityAttributes`.`' + name + '` AS `EntityAttributes.' + name + '`').join(','),
            'FROM (',
            'SELECT',
            [ 'id', ...Object.keys(Entity.attributes) ].map((name) => '`Entity`.`' + name + '`').join(','),
            'FROM `Entities` AS `Entity` ',
            ') AS `Entity` LEFT OUTER JOIN `EntityAttributes` AS `EntityAttributes` ON `Entity`.`id` = `EntityAttributes`.`EntityId`',
          ...
          let countSQL = [
            'SELECT count(DISTINCT(`Entity`.`id`)) AS `count` ',
            'FROM `Entities` AS `Entity`',
