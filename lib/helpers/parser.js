      ...
      case OBJECT: {
        ...
        for (const subName of subNames) {
          ...
          if (isOperator) {
            ...
            console.warn(`[ClaryDriverSequelize] Passing nested filter is not supported: "${name}.${subName}"`)
            if (subName === 'id') {
              console.warn('[ClaryDriverSequelize] If you want to filter by entity, use ref string instead of id (eg. `filter:{user: {$ref: "User#01"}}`)')
            }
          }
        }
        break
