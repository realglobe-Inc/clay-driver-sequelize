      ...
      case OBJECT: {
        ...
        for (let subName of subNames) {
          ...
          if (isOperator) {
            ...
            parsed[ name ][ subName ] = serialize(typeOf(subValue), subValue)
          } else {
            parsed[ `${name}.${subName}` ] = subValue
          }
        }
        break
      }
            ...
            case OBJECT: {
              let subValues = Object.keys(value)
                .reduce((subValues, subKey) => Object.assign(subValues, {
                  [`${name}.${subKey}`]: serialize(type, value[ subKey ])
                }), {})
