...
class SequelizeDriver extends Driver {
  ...
  async getEntityModels (resource) {
    ...
    const EntityAttribute = entityAttributeModel({ db, resourceName, prefix })
    const Entity = entityModel({ db, resourceName, prefix })
    await Resource.sync()
    await Entity.sync()
    await EntityAttribute.sync()
    await asleep(10)
  ...
  async one (resourceName, id) {
    ...
    const { Entity, EntityAttribute } = await s.getEntityModels(resource)
    const entityAttributes = await EntityAttribute.allOf()
    const cid = String(id)
  ...
  async create (resourceName, attributes) {
    ...
    const cid = String(id)
    const cols = await EntityAttribute.colsFor(attributes)
    const entity = await Entity.create(Object.assign(
      { cid },
      Entity.attributesWithCols(cols)
    ))
  ...
  async update (resourceName, id, attributes) {
    ...
    const cols = await EntityAttribute.colsFor(attributes)
    await entity.update(Entity.attributesWithCols(cols))
    return s.one(resourceName, entity.cid)
  }
  ...
  async drop (resourceName) {
    ...
    const { Entity } = await s.getEntityModels(resource)
